   ...
   ! copy from 3D array to 2D slab
       ...
       DO kz = kts,kte
        DO ix = its,ite
          ...
          IF ( ipconc >= 5 ) THEN
             an(ix,1,kz,lnc)  = Min( qccn, ccw(ix,kz,jy))
...
!     write(0,*) 'N2M: done load an, jy,lccn = ',jy,lccn,qccn
...
! Following Greg Thompson, calculation for effective radii. Used by RRTMG LW/SW schemes if enabled in module_physics_init.F
     ...
     write(0,*) 'call eff_radius'
          call calc_eff_radius   &
     &         (nx,ny,nz,na,jy & 
     &          ,nor,nor & 
     &          ,t1,t2,t3  & 
     &          ,an,dn1 )
     write(0,*) 'done eff_radius'
   ...
   ! copy 2D slabs back to 3D
       ...
       DO kz = kts,kte
        DO ix = its,ite
         ...
         IF ( present( tt ) ) THEN
           tt(ix,kz,jy) = t0(ix,1,kz) ! an(ix,1,kz,lt)*t77(ix,1,kz) ! t0 was updated in nucond
         ELSE
...
!
!
      if ( ndebug .gt. 0 ) write(0,*) 'ventilation rain'
...
!
      if ( ndebug .gt. 0 ) write(0,*) 'ventilation snow'
...
!
!
      if ( ndebug .gt. 0 ) write(0,*) 'ventilation graupel'
...
!     &            Sqrt(axh(mgs)*rhovt(mgs)) )
      ...
      if ( ndebug .gt. 0 ) write(0,*) 'ventilation hail'
...
!
     ...
     &  (1.0/(fav(mgs)+fbv(mgs)))
      ...
      if ( ndebug .gt. 0 ) write(0,*) 'graupel melting'
...
!      chmlrr(mgs) = chmlr(mgs)
      ...
      if ( ndebug .gt. 0 ) write(0,*) 'ice sub/dep'
...
!
      ...
      if ( ndebug .gt. 0 ) write(0,*) 'sublimation limit'
